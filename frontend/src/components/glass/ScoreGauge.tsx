...
const sizeConfig = {
  ...
  xl: { size: 200, stroke: 12, fontSize: 'text-5xl', labelSize: 'text-lg' },
}
  ...
  return (
    <div className={cn('relative inline-flex flex-col items-center', className)}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        {showLabel && (
          <span className={cn('font-medium', config.labelSize, colors.textClass)}>
            {label || colors.label}
          </span>
        )}
      </div>
    ...
    <div className={cn('w-full', className)}>
      ...
      <div className="h-2 bg-white/[0.1] rounded-full overflow-hidden backdrop-blur-sm">
        <motion.div
          ...
          initial={{ width: 0 }}
          animate={{ width: `${score}%` }}
          transition={{ duration: animated ? 1 : 0, ease: [0.16, 1, 0.3, 1] }}
          style={{
...
export function ModuleScore({
  ...
  return (
    <div
      ...
      style={{
        boxShadow: `0 4px 20px rgba(0,0,0,0.2), 0 0 20px ${colors.glow.replace('0.5', '0.1')}`,
      }}
    >
