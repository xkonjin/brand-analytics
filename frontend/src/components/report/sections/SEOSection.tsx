...
export function SEOSection({ data, className = '' }: SEOSectionProps) {
  ...
  const metrics = [
    {
      ...
      unit: '/100',
      trend: (data.performance_score ?? 0) >= 90 ? 'up' : (data.performance_score ?? 0) >= 50 ? 'stable' : 'down',
    },
    {
      label: 'Accessibility',
  ...
  const webVitals = [
    { label: 'FCP', value: formatMs(data.first_contentful_paint), subtitle: 'First Contentful Paint' },
    { label: 'LCP', value: formatMs(data.largest_contentful_paint), subtitle: 'Largest Contentful Paint' },
    { label: 'TBT', value: `${data.total_blocking_time ?? 0}ms`, subtitle: 'Total Blocking Time' },
    { label: 'CLS', value: formatCLS(data.cumulative_layout_shift), subtitle: 'Cumulative Layout Shift' },
  ];
