...
class DataSource(BaseModel):
    ...
    name: str = Field(..., description="Data source name (e.g., 'Google PageSpeed', 'Moz API')")
    ...
    version: Optional[str] = Field(None, description="API version or data format version")
    reliability_score: Optional[float] = Field(None, ge=0, le=1, description="Data source reliability (0-1)")


class ConfidenceFactors(BaseModel):
    """Factors that contribute to confidence calculation."""
    data_completeness: float = Field(..., ge=0, le=1, description="How complete the data is (0-1)")
    data_freshness: float = Field(..., ge=0, le=1, description="How recent the data is (0-1)")
    source_reliability: float = Field(..., ge=0, le=1, description="Reliability of data sources (0-1)")
    methodology_robustness: float = Field(..., ge=0, le=1, description="How robust the scoring method is (0-1)")
...
class BenchmarkComparison(BaseModel):
    ...
    difference_from_benchmark: float = Field(..., description="Difference from benchmark (can be negative)")
    benchmark_category: str = Field(..., description="Benchmark category (e.g., 'B2B SaaS', 'E-commerce')")
    benchmark_year: Optional[int] = Field(None, description="Year the benchmark data is from")


class ProvenanceRecord(BaseModel):
    """Complete provenance information for a score."""
    score_id: UUID = Field(default_factory=uuid.uuid4, description="Unique identifier for this score calculation")
    analyzer_version: str = Field(..., description="Version of the analyzer that produced this score")
    ...
    calculation_steps: List[str] = Field(default_factory=list, description="Step-by-step calculation process")
    assumptions_made: List[str] = Field(default_factory=list, description="Assumptions made during scoring")
    limitations: List[str] = Field(default_factory=list, description="Known limitations of this score")
...
class NormalizedScore(BaseModel):
    ...
    value: float = Field(..., ge=0, le=100, description="The normalized score value (0-100)")

    # Enhanced metadata (optional for backward compatibility)
    confidence_level: Optional[ConfidenceLevel] = None
    confidence_score: Optional[float] = Field(None, ge=0, le=1, description="Numerical confidence score (0-1)")
...
class EnhancedScoreCard(BaseModel):
    ...
    normalized_module_scores: Optional[Dict[str, NormalizedScore]] = Field(default_factory=dict)
...
class BaseScorer:
    ...
    def create_normalized_score(
        ...
        if normalization_method and benchmark_value:
            normalized_value = self.normalize_score(raw_score, normalization_method, benchmark_value)
        else:
