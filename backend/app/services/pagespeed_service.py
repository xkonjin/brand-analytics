...
class PageSpeedService:
            ...
            try:
                async with httpx.AsyncClient(timeout=self.TIMEOUT) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        # Cache the result
                        await cache.set(cache_key, self._result_to_dict(result), ttl=3600 * 6)
                        return result
                ...
                if attempt < self.MAX_RETRIES:
                    logger.warning(f"PageSpeed timeout, retrying ({attempt + 1}/{self.MAX_RETRIES})")
                    continue
                return PageSpeedResult(
                    success=False,
                    url=url,
    ...
    def _result_to_dict(self, result: PageSpeedResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "url": result.url,
            "strategy": result.strategy.value if isinstance(result.strategy, Strategy) else result.strategy,
            "performance_score": result.performance_score,
            "accessibility_score": result.accessibility_score,
            "best_practices_score": result.best_practices_score,
            "seo_score": result.seo_score,
            "core_web_vitals": asdict(result.core_web_vitals) if result.core_web_vitals else None,
