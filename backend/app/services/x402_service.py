...
class X402Service:
    ...
    async def create_invoice(
        self, payer_address: str, db: AsyncSession
    ) -> Dict[str, Any]:
        """
        ...
        deadline_dt = datetime.now(timezone.utc) + timedelta(
            minutes=settings.PAYMENT_DEADLINE_MINUTES
        )
    ...
    async def process_payment(
        ...
        try:
            tx_hash = await self._submit_to_relayer(
                authorization, full_signature, client_ip
            )
    ...
    async def _submit_to_relayer(
        self, authorization: Dict, signature: str, client_ip: str
    ) -> str:
        """
