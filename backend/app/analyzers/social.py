...
class SocialMediaAnalyzer(BaseAnalyzer):
    ...
    async def analyze(self) -> AnalyzerResult:
        ...
        try:
            ...
            result_data = {
                ...
                "platforms_dormant": len(
                    [p for p in platforms if not self._is_active(p)]
                ),
                "platforms_missing": self._get_missing_platforms(social_links),
                "overall_engagement_rate": self._raw_data["summary"].get(
                    "avg_engagement"
                ),
        ...
        if analysis.success and analysis.user:
            ...
            self._raw_data["twitter_analysis"] = {
                ...
                "user": {
                    ...
                    "created_at": analysis.user.created_at.isoformat()
                    if analysis.user.created_at
                    else None,
                },
                "tweets": [
                    {
    ...
    def _calculate_summary(
        self, platforms: List[SocialPlatformMetrics]
    ) -> Dict[str, Any]:
