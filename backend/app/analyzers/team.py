...
from app.models.report import Finding, Recommendation, SeverityLevel, TeamMember
...
class TeamPresenceAnalyzer(BaseAnalyzer):
    ...
    def _analyze_team_page(self) -> Dict[str, Any]:
        ...
        has_team = any(t in nav_texts for t in ["team", "about", "about us", "who we are"])
    ...
    def _calculate_score(self) -> float:
        ...
        score = 40  # Base
        if team.get("exists"): score += 25
        if team.get("founder_count", 0) > 0: score += 15
        if linkedin.get("exists"): score += 20
